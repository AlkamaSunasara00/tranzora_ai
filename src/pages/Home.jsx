    ...
    <div className="page-container">
      <div className="background-effects">
        <div style={{ width: "100vw", height: "100vh", position: "absolute", inset: 0 }}>
          <Prism
            animationType="hover"
            timeScale={isMobile ? 0.2 : 0.5}  // Slower on mobile
            height={isMobile ? 2 : 5}         // Reduced complexity
            baseWidth={isMobile ? 1.5 : 5.5}  // Smaller on mobile
            scale={isMobile ? 2 : 3.6}        // Reduced scale
            hueShift={0}
            colorFrequency={isMobile ? 0.1 : 1} // Less color variation
            noise={isMobile ? 0.3 : 0.5}      // Reduced noise
            glow={isMobile ? 0.5 : 1}         // Reduced glow
          />
                ...
                <div className={uploadAreaClasses} onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); setIsDragOver(true) }} onDragLeave={() => setIsDragOver(false)} onClick={() => !isProcessing && !file && fileInputRef.current?.click()}>
                  ...
                  {isProcessing ? (
                    <div className="processing-view">
                      <Loader />
                      <p className="status-text processing">Please wait! it's Processing...</p>
                    </div>
                  ) : file ? (
